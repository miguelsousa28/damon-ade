...
async function startServer(): Promise<void> {
	...
	// Check if socket is live before removing it. On Windows this is a named
	// pipe, not a filesystem entry, so probing the connection is authoritative.
	if (TERMINAL_HOST_USES_NAMED_PIPE || existsSync(SOCKET_PATH)) {
	...
	await new Promise<void>((resolve, reject) => {
		...
		newServer.listen(SOCKET_PATH, () => {
			...
			if (!TERMINAL_HOST_USES_NAMED_PIPE) {
				try {
